      ...
      <el-table-column label="操作" width="210" align="center">
        <template scope="scope">
          <el-tooltip content="停用" placement="top" v-if="scope.row.vstatus==1">
            <el-button class="fa fa-stop-circle-o " aria-hidden="true" type="primary" size="mini"   @click="stopVstatus(scope.row.id,scope.$index)" :disabled="platformstop ? false:true" ></el-button>
          </el-tooltip>
          <el-tooltip content="开启" placement="top" v-else>
            <el-button class="fa fa-play-circle-o" type="primary" size="mini"  @click="startVstatus(scope.row.id,scope.$index)" :disabled="platformstart&&scope.row.vstatus==2 ? false:true" ></el-button>
          </el-tooltip>
          
          <el-tooltip :content="scope.row.vstatus==2?'编辑':'查看'" placement="top">
            <el-button :class="scope.row.vstatus==2?'el-icon-edit':'el-icon-view'" type="primary"  size="mini" @click="modifyItem(scope.row.id,scope.row.vstatus)" :disabled="platformupdate ? false:true"></el-button>
          </el-tooltip>
          <el-tooltip content="删除" placement="top">
            <el-button class="el-icon-delete" type="primary" size="mini" @click="removeItem(scope.row.id)" :disabled="platformdelete&&scope.row.vstatus==2 ? false:true"></el-button>
          </el-tooltip>
          <el-tooltip content="复制链接" placement="top">
            <el-button class="el-icon-share" type="primary" size="mini" :disabled="platformcopy ? false:true" @click="showUrl(scope.row.id,scope.row.isShared)"></el-button>
          </el-tooltip>
        </template>
      </el-table-column>
    </el-table>
  ...
  export default {
    ...
    methods:{
      ...
      modifyItem(id,status){ 
        this.$router.push({path:'/home/coupon/addplatform',query:{id:id,status:status}})
      },
      //点击停用
